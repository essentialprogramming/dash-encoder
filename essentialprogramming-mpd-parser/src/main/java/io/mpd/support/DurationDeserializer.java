...
import io.mpd.support.util.ParseUtil;
...
public class DurationDeserializer extends JsonDeserializer<Duration> {
    ...
    static long parseFraction(CharSequence text, int start, int end, boolean negate) {
        ...
        long fraction = ParseUtil.parseLong(text, start, end, 10);
    ...
    private Duration parse(String text) {
        ...
        if (yearStart >= 0) {
            years = ParseUtil.parseInt(text, yearStart, yearEnd, 10);
        }

        if (monthStart >= 0) {
            months = ParseUtil.parseInt(text, monthStart, monthEnd, 10);
        }

        if (dayStart >= 0) {
            days = ParseUtil.parseInt(text, dayStart, dayEnd, 10);
        }

        if (hourStart >= 0) {
            hoursAsSecs = Math.multiplyExact(3600L, ParseUtil.parseInt(text, hourStart, hourEnd, 10));
        }

        if (minuteStart >= 0) {
            minutesAsSecs = Math.multiplyExact(60L, ParseUtil.parseInt(text, minuteStart, minuteEnd, 10));
        }
